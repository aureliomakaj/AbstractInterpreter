...
let rec pretty_stm (stm: stm) = 
    ...
    | IfThenElse (expr, stm1, None) -> sprintf "if %s THEN %s" (pretty_expr expr) (pretty_stm stm1)
    | IfThenElse (expr, stm1, Some stm2) -> sprintf "if ( %s ) THEN \n %s \n ELSE %s " (pretty_expr expr) (pretty_stm stm1) (pretty_stm stm2)
